...
//Functions for Growing Mode Calculation

double W_7CPL(double z)		//Dark Energy Parametrization
{
	double ans;
...
double calcHRadIntegrand(double z)	//Inner Integrand for H(z)
{
	double ans;
	ans = 1.0 + W_7CPL(z);
	ans /= (1.0 + z);
...
double calcHRad(double endz)		//Inner Integral for H(z)
{
	double upper, lower;	
	lower = 0.0;
	upper = endz;
...
double calcH(double z)			//Calulator for H(z)
{
	double ans;
	ans = omega[2] * pow((1+z), 3);
	ans += (1-omega[2]) * exp(3*calcHRad(z));
	ans = pow(ans, 0.5);
	ans *= H0;
	return ans;
...
double calcIntegrand(double x)		//Alternate Integrand for growing mode
{
	double ans = 1.0;
	ans /= pow(x, 3);
...
double calcIntegral(double a)		//Alternate Integral for Growing Mode
{
	double upper, lower;	
	lower = 0.0;
	upper = a;
...
double getAfromZ(double z)		//Conversion of a to z
{
	double ans;
	ans = 1.0/(1.0 + z);
	return ans;
...
double calcdelta(double z)		//Alternate Growing Mode
{	
...
int main()
{
	cout<<"Enter 1. for Default values (H = 69.6, DM = 0.3, Lambda = 0.7, z = 0, Nk = 100)\n";
	...
	mat transfer_values = mat(Nk, 2);
	float k;
	float alpha = ((float)log10(kmax/kmin))/(Nk-1);
	...
	{
		k = A*pow(10, i*alpha);
		transfer_values(i-1, 0) = k;
		float z = TFfit_onek(k*h, NULL, NULL);
		transfer_values(i-1, 1) = rscaling*rscaling*k*z*z;
	}
	string str = "transfer_function.txt";
	transfer_values.save(str, raw_ascii);

	Nz = 1000;	
	mat deltaG = mat(Nz+1, 2);
	double za = 0.0;
	double zb = 10.0;
	double temp = za;
	double hstep = (zb - za)/Nz;
	...
	for (int i=0; i<=Nz; i++)
	{
		deltaG(i, 0) = temp;
		if (i%10 == 0)
			cout<<(float)i*100.0/(float)Nz<<"% done\n";
		deltaG(i, 1) = calcdelta(temp);
		temp = temp + hstep;
	}
	string str2 = "newRkData.txt";
	deltaG.save(str2, raw_ascii);
}
